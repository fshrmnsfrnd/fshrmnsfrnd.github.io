...
interface Box { id: number; x: number; y: number; vy: number; falling: boolean; }

interface Player { x: number; y: number; vx: number; vy: number; w: number; h: number; canJump: boolean; alive: boolean; }

// Grid nur für ruhende Boxen (fallende sind nicht eingetragen bis sie landen)
const grid: (Box | null)[][] = Array.from({ length: ROWS }, () => Array.from({ length: COLS }, () => null));
let boxes: Box[] = [];
let nextBoxId = 1;

// Spieler Start unten links mittig
let player: Player = { x: Math.floor(COLS / 2), y: ROWS - 1, vx: 0, vy: 0, w: 1, h: 1, canJump: false, alive: true };
...
function layout() {
	...
	const controls = document.getElementById('touchControls') as HTMLElement | null;
	...
	const safeBottom = (window as any).visualViewport ? (window as any).visualViewport.height - window.innerHeight : 0;
...
// Highscore laden
try { const stored = localStorage.getItem(HS_KEY); if (stored) highScore = parseInt(stored) || 0; } catch { /* ignore */ }
hsEl.textContent = highScore.toString();

// Eingaben (Keyboard) – kein Halten für Horizontal
window.addEventListener('keydown', e => {
	if (e.repeat) return; // kein Dauerfeuer durch Halten
	switch (e.key) {
		case 'ArrowLeft': case 'a': case 'A': attemptMove(-1); break;
		case 'ArrowRight': case 'd': case 'D': attemptMove(1); break;
		case 'ArrowUp': case 'w': case 'W': case ' ': queueJump(); break;
	}
});
...
function cellBlocked(x: number, y: number): boolean {
	if (x < 0 || x >= COLS || y >= ROWS) return true; // Wand / Boden blockiert
	if (y < 0) return false; // über dem Spielfeld
	return !!grid[y][x];
}

function landBox(b: Box) {
	b.falling = false;
	b.vy = 0;
...
function spawnBox() {
	...
	const box: Box = { id: nextBoxId++, x, y: -1, vy: 0, falling: true };
	boxes.push(box);
}
...
function update(dt: number, now: number) {
	...
	if (onGround) { player.vy = 0; player.canJump = true; hangTimer = 0; }
	else {
		if (player.vy < 0) { // Aufstieg -> langsamer abbremsen
			...
			if (player.vy >= -0.0001) player.vy = 0;
	...
	if (player.vy > 0) { // nach unten
		...
		while (newY > player.y) {
			if (player.y + 1 >= ROWS || grid[player.y + 1][player.x]) { newY = player.y; break; }
			player.y += 1;
		}
		...
		if (newY < player.y) {
			...
			if (player.y - 1 >= 0 && grid[player.y - 1][player.x]) {
				player.vy = 0; newY = player.y; }
					...
					if (targetY < minY) { targetY = minY; player.vy = 0; }
					player.y = targetY;
				}
		}
	}
...
function drawGridBoxes() {
	for (let y=0; y<ROWS; y++) {
		for (let x=0; x<COLS; x++) {
			const b = grid[y][x];
			if (b) drawBox(x, y, false);
		}
	}
}
function drawFallingBoxes() {
	for (const b of boxes) if (b.falling) drawBox(b.x, b.y, true);
}

function drawBox(x: number, y: number, falling: boolean) {
	const px = x * cellSize;
	const py = y * cellSize;
	ctx.save();
	ctx.translate(px, py);
...
function attemptMove(dir: -1 | 1) {
	if (!player.alive) return;
	const targetX = player.x + dir;
	if (targetX < 0 || targetX >= COLS) return;
	...
	if (falling) {
		...
		if (beyondX < 0 || beyondX >= COLS) return;
		if (landedBoxAt(beyondX, Math.round(falling.y))) return;
	...
	if (player.y >= 0 && player.y < ROWS) {
		...
		if (landed) {
			...
			if (boxAbove(targetX, player.y)) return;
	...
	// frei -> bewegen
	if (!cellBlocked(targetX, player.y)) player.x = targetX;
}

function rangesOverlap(a1: number, a2: number, b1: number, b2: number) { return !(a2 <= b1 || a1 >= b2); }
function landedBoxAt(x: number, y: number): boolean { return y >= 0 && y < ROWS && !!grid[y][x]; }
function boxAbove(x: number, y: number): boolean {
	// y = Ebene der zu schiebenden Box
	const aboveY = y - 1;
	if (aboveY < 0) return false;
	// Landed box directly above?
	if (grid[aboveY][x]) return true;
...
function hasSupportBelow(x: number, y: number): boolean {
	const belowY = y + 1;
	if (belowY >= ROWS) return true; // Boden
