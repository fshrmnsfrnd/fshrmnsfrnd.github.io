...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const ctx = gameCanvas === null || gameCanvas === void 0 ? void 0 : gameCanvas.getContext("2d");
    // Start game
    main();
        ...
        if (e.changedTouches.length === 1) {
            ...
            if (Math.abs(dxTouch) > 30 || Math.abs(dyTouch) > 30) {
                if (Math.abs(dxTouch) > Math.abs(dyTouch)) {
                    // Horizontaler Swipe
                    if (dxTouch > 0) {
                        changeDirection({ keyCode: 39 }); // rechts
                    }
                    else {
                        changeDirection({ keyCode: 37 }); // links
                    }
                }
                else {
                    // Vertikaler Swipe
                    if (dyTouch > 0) {
                        changeDirection({ keyCode: 40 }); // runter
                    }
                    else {
                        changeDirection({ keyCode: 38 }); // hoch
                    }
                }
            }
        }
