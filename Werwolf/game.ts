...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (!(players.some((player: { role: { name: string } }) => player.role.name === "Hexe"))) {
        gamePhases.splice(3, 1)
        if (!(players.some((player: { role: { name: string } }) => player.role.name === "Seher"))) {
    ...
    players.forEach((player: { player: string, role: { name: string } }) => {
    ...
    function updateGameDisplay(text: string): void {
