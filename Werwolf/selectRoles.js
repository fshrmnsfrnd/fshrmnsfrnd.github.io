...
function assignRoles() {
    ...
    roles.forEach((role, index) => {
        const inputElem = document.querySelector(`input[name="role-${role.name}-count"]`);
        ...
        const storedRolesCount = rolesCountString ? JSON.parse(rolesCountString) : [];
    ...
    if (rolesCountString) {
        ...
        if (storedRolesCount && storedRolesCount.length > 0) {
            storedRolesCount.forEach(([role, number]) => {
                let roleCountInputElement = document.getElementById(`role-${role.name}-count`);
